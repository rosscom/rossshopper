 ...
 */
@DefaultProperty(value = "password")
public class PasswordFieldExtended extends PasswordField {
    private static final StyleablePropertyFactory<PasswordFieldExtended> FACTORY                       = new StyleablePropertyFactory<>(TextField.getClassCssMetaData());
    private static final Color                                     DEFAULT_MATERIAL_DESIGN_COLOR = Color.web("#ffffff");
    private static final Color                                     DEFAULT_PROMPT_TEXT_COLOR     = Color.web("#ffffff");
    ...
    private static final CssMetaData<PasswordFieldExtended, Color>       MATERIAL_DESIGN_COLOR         = FACTORY.createColorCssMetaData("-material-design-color", s -> s.materialDesignColor, DEFAULT_MATERIAL_DESIGN_COLOR, false);
    private static final CssMetaData<PasswordFieldExtended, Color>       PROMPT_TEXT_COLOR             = FACTORY.createColorCssMetaData("-prompt-text-color", s -> s.promptTextColor, DEFAULT_PROMPT_TEXT_COLOR, false);
    ...
    // ******************** Constructors **************************************
    public PasswordFieldExtended() {
        this("");
    }
    public PasswordFieldExtended(final String TEXT) {
    //    super(TEXT);
        ...
        fontSize            = new SimpleDoubleProperty(PasswordFieldExtended.this, "fontSize", getFont().getSize());
    ...
    private void handleTextAndFocus(final boolean IS_FOCUSED) {
        ...
        if (IS_FOCUSED | LENGTH > 0 || isDisabled() || !isEditable()) {
    ...
    // ******************** Style related *************************************
    @Override public String getUserAgentStylesheet() { return PasswordFieldExtended.class.getResource("login.css").toExternalForm(); }
